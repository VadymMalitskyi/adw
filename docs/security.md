...
The managed runtime drops every container capability and adds back only `CHOWN`, `KILL`, `SETUID`, `SETGID`, and `NET_ADMIN` for the two tightly scoped sudo entrypoints, credential-volume ownership, proxy lifecycle, and firewall setup. It does not grant `SYS_ADMIN`, `SYS_PTRACE`, `NET_RAW`, an unconfined seccomp/AppArmor profile, or a setuid `bwrap`. The security workflow builds both pinned agents and exercises the firewall, direct-egress denial, hostname denial, SNI mismatch denial, the constrained Claude page-fetch path, and rejection of ordinary clients on that path under the exact runtime profile. Nested bubblewrap availability varies by container host; ADW does not weaken the outer container to make it available. Provider sandboxes must fail closed or fall back to their permission prompts when nested namespaces are unavailable. `NET_ADMIN` is still powerful inside the container's network namespace, and the root firewall entrypoint remains trusted code; keep the container runtime and host patched and treat the container as defense in depth rather than a VM boundary.
