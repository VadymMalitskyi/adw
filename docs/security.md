...
- Codex workers use the active project policy without a runner-supplied sandbox override, ignore flag, or approval-bypass flag. Claude stages run as in-session subagents under the active session's identity, and never fall back to `claude -p` or an API-key route. A review stage is told to be read-only, but the enforceable check is `adw execution-assert-target`, which compares a digest over the changed files' bytes against the snapshot taken before the review ran — an edit is caught whether or not it changes what `git status` prints.
