...
- Codex workers use the active project policy without a runner-supplied sandbox override, ignore flag, or approval-bypass flag. Claude stages run as in-session subagents under the active session's identity, and never fall back to `claude -p` or an API-key route. The Claude Workflow removes direct editing tools from review agents and has every stage invoke `adw execution-assert-target` through a mode-`0600`, SHA-256-bound temporary copy of the preflight envelope. The gate compares a digest over the changed files' bytes against the snapshot taken before a review, so an edit is detected whether or not it changes what `git status` prints. Because the Workflow runtime cannot invoke tools directly, Claude's stage result remains candidate evidence; the shared post-workflow finalizer is authoritative for HEAD, scope, non-target drift, and validation.
