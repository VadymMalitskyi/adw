...
The coordinator selects exactly one provider route. Codex uses a dependency-free Node host that launches supported noninteractive `codex exec` workers under the active project policy. Claude uses a native dynamic Workflow that launches fresh in-session subagents and has every stage invoke `adw execution-assert-target`; it never falls back to `claude -p`, changes credentials, or introduces API billing. Independent groups settle concurrently; each follows implementation → fresh review → optional fix → fresh re-review, with no more than two fix/re-review cycles. Provider output is schema-validated but remains a candidate, not authoritative execution success.
...
Codex can run authoritative Git gates directly between worker subprocesses. A Claude Workflow cannot invoke tools itself, so each stage subagent runs its gate and the shared finalizer remains authoritative after the workflow. The routes therefore share the final contract but do not have identical inter-stage assurance. Review subagents have direct editing tools removed and must prove their snapshot did not change, while the finalizer independently catches wrong checkouts, commits, scope escapes, and non-target drift. Workers never commit, push, create tracker items, or create pull requests. The coordinator owns Git and every external action; no result proves cross-branch integration.
