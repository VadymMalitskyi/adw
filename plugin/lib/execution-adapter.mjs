...
export function stageResultSchema(stage) { return { type: "object", additionalProperties: false, required: ["status", "findings", "stage"], properties: { status: { enum: ["passed", "failed"] }, findings: { type: "array", items: { type: "object", additionalProperties: false, required: ["severity", "summary"], properties: { severity: { enum: ["high", "medium", "low"] }, summary: { type: "string", maxLength: 2048 } } } }, stage: { type: "string", const: stage } } }; }
