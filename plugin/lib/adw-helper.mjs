...
function computeApprovalBundle(inputs) {
...
function createApprovalBundle({ approver, approved_at, plugin_version, docs_commit, inputs }) {
...
async function loadArtifactFile({ project_root, path, artifact }) {
  ...
  try {
    ...
    if (error.code === "ENOENT") throw new PathError(`artifact does not exist: ${path}`);
    throw error;
  }
...
async function loadArtifactSchema(artifact, version) {
...
async function validateArtifact(artifact, data) {
  ...
  let schema;
  try {
    schema = await loadArtifactSchema(artifact, data.schema);
  } catch (error) {
    return { valid: false, errors: [{ path: "/schema", keyword: "version", message: error.message }] };
  }
  const result = validateJsonSchema(schema, data);
  if (artifact === "plan" && Array.isArray(data?.tasks)) {
    data.tasks.forEach((task, index) => {
      if (task?.id !== index + 1) result.errors.push({ path: `/tasks/${index}/id`, keyword: "sequence", message: `must be ${index + 1} so tasks execute sequentially` });
    });
    ...
    if (tracker && tracker.profile === void 0 !== (tracker.profile_digest === void 0)) result.errors.push({ path: "/effective_policy/work_tracker", keyword: "profile", message: "profile and profile_digest must appear together" });
    if (tracker && tracker.child_profile === void 0 !== (tracker.child_profile_digest === void 0)) result.errors.push({ path: "/effective_policy/work_tracker", keyword: "profile", message: "child_profile and child_profile_digest must appear together" });
  ...
  if (result.valid && artifact === "approval") {
    const invalidationFields = [data.invalidated_at, data.invalidation_reason, data.replaced_by].filter((value) => value !== void 0);
  ...
  if (result.valid && artifact === "integration") {
    ...
    for (const [index, binding] of data.bindings.entries()) {
      const hasDigest = binding.requirements_digest !== void 0;
      const hasFields = binding.requirement_fields !== void 0;
  ...
  if (result.valid && artifact === "work-item-profile") {
    ...
    const declared = /* @__PURE__ */ new Set([...data.required_fields ?? [], ...data.allowed_fields ?? [], ...Object.keys(data.defaults ?? {})]);
...
function recordValidation({ change_id, plugin_version, code_commit, docs_commit, recorded_at, commands = [], deferred = [] }) {
...
function resolvedValidation(item, sourcePath, defaultCwd = ".") {
  ...
  if (typeof item === "string") return { command, cwd: defaultCwd, timeout_ms: 12e4, required: true, source: sourcePath };
  return { command, cwd: item.cwd ?? defaultCwd, timeout_ms: item.timeout_ms ?? 12e4, required: item.required !== false, source: item.source };
}
function validateWorkItemPayload(profile, payload) {
  const errors = [];
  ...
  const allowed = /* @__PURE__ */ new Set([...profile.required_fields ?? [], ...profile.allowed_fields ?? [], ...Object.keys(profile.defaults ?? {})]);
...
function resolveProjectPolicy({ project, affected_paths, profiles = {} }) {
...
function resolveValidationSet({ effective_policy, tasks }) {
...
function computeRequirementsDigest(fields) {
...
function computeAuthorizationDigest({ target, operation, payload }) {
...
function recordExternalAction(input) {
  ...
  if (authorization !== void 0) receipt.authorized_by = typeof authorization === "string" ? authorization : authorization.actor;
  ...
  } else if (input.authorization_digest !== void 0) receipt.authorization_digest = input.authorization_digest;
  for (const key of ["external_id", "url", "before_revision", "after_revision"]) if (input[key] !== void 0) receipt[key] = input[key];
  return receipt;
}
...
async function dispatch(command, rawInput) {
  ...
  switch (command) {
    ...
    case "verify-approval-bundle": {
      ...
      const commitMatches = input.docs_commit === void 0 || input.docs_commit === input.approval?.docs_commit;
