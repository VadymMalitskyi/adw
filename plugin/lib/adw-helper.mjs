...
var EXECUTION_MODES = Object.freeze(["orchestrated", "sequential"]);
...
var RUNTIMES = /* @__PURE__ */ new Set(["node", "python", "go", "rust", "java", "ruby", "dotnet"]);
var SECRET_LIKE_KEY = /(?:password|passwd|token|api[_-]?key|secret|credential|authorization|cookie|private[_-]?key)/i;
var IDENTIFIER = /^[a-z0-9](?:[a-z0-9_-]*[a-z0-9])?$/;
var CHANGE_ID = /^[a-z0-9](?:[a-z0-9_-]|\.[a-z0-9_-]+)*$/;
...
function parseYaml(source, label = "YAML document") {
  if (typeof source !== "string" && !Buffer.isBuffer(source)) throw new InputError(`${label} must be UTF-8 text`);
  let text;
  try {
    text = Buffer.isBuffer(source) ? new TextDecoder("utf-8", { fatal: true }).decode(source) : source;
  } catch (error) {
    throw new InputError(`${label} is not valid UTF-8: ${error.message}`, { cause: error });
  }
  const document = (0, import_yaml.parseDocument)(text, { merge: false, prettyErrors: false, strict: true, uniqueKeys: true, version: "1.2" });
...
function checkNonEmptyString(errors, value, path, maximum = 4e3) {
...
function checkSingleLine(errors, value, path, maximum = 2e3) {
...
function validateValidationCommand(errors, item, path, componentPath) {
  ...
  checkKnownKeys(errors, item, /* @__PURE__ */ new Set(["command", "cwd", "timeout_ms", "required", "source"]), path);
  ...
  if (item.required !== void 0 && typeof item.required !== "boolean") return errors.add(`${path}/required`, "must be a boolean") && null;
  if (item.source !== void 0) checkSingleLine(errors, item.source, `${path}/source`);
  const normalized = { command: item.command, cwd, timeout_ms: timeout, required: item.required !== false };
  if (item.source !== void 0) normalized.source = item.source;
  return normalized;
}
function validateComponents(errors, value, normalized) {
  ...
  for (const [id, raw] of entries) {
    ...
    if (!IDENTIFIER.test(id)) {
      errors.add(path, "component id must be lowercase alphanumeric with `-` or `_` separators");
      continue;
    }
    if (!checkObject(errors, raw, path)) continue;
    checkKnownKeys(errors, raw, /* @__PURE__ */ new Set(["path", "validate"]), path);
    ...
    if (raw.validate !== void 0) {
      if (!Array.isArray(raw.validate)) {
        errors.add(`${path}/validate`, "must be an array of commands");
        continue;
      }
...
function validateProviders(errors, value, normalized) {
  ...
  for (const [capability, raw] of Object.entries(value)) {
    ...
    if (!checkObject(errors, raw, path)) continue;
    checkKnownKeys(errors, raw, /* @__PURE__ */ new Set(["provider", "required", "transport", "access", "settings"]), path);
    if (!checkSingleLine(errors, raw.provider, `${path}/provider`, 100)) continue;
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(raw.provider)) {
      errors.add(`${path}/provider`, "must be a lowercase provider name such as github");
      continue;
    }
    const entry = { provider: raw.provider, required: false };
    if (raw.required !== void 0) {
      if (typeof raw.required !== "boolean") {
        errors.add(`${path}/required`, "must be a boolean");
        continue;
      }
      entry.required = raw.required;
    }
    if (raw.transport !== void 0) {
      if (!TRANSPORTS.has(raw.transport)) {
        errors.add(`${path}/transport`, `must be one of: ${[...TRANSPORTS].join(", ")}`);
        continue;
      }
      entry.transport = raw.transport;
    }
    if (raw.access !== void 0) {
      if (!ACCESS_MODES.has(raw.access)) {
        errors.add(`${path}/access`, `must be one of: ${[...ACCESS_MODES].join(", ")}`);
        continue;
      }
    ...
    if (raw.settings !== void 0) {
      ...
      for (const [key, setting] of Object.entries(raw.settings)) {
        if (SECRET_LIKE_KEY.test(key)) {
          errors.add(`${path}/settings/${key}`, "credential-like settings are forbidden; keep credentials in the provider or client credential store");
          continue;
        }
...
function validateProjectConfig(data) {
  ...
  if (data.git === void 0) errors.add("/git", "is required");
    ...
    checkKnownKeys(errors, data.git, /* @__PURE__ */ new Set(["base_branch"]), "/git");
    if (checkBranchName(errors, data.git.base_branch, "/git/base_branch")) normalized.git.base_branch = data.git.base_branch;
  }
  if (data.docs === void 0) errors.add("/docs", "is required");
  else if (checkObject(errors, data.docs, "/docs")) {
    checkKnownKeys(errors, data.docs, /* @__PURE__ */ new Set(["branch", "worktree", "sync_marker"]), "/docs");
    ...
    if (data.docs.sync_marker !== void 0) {
  ...
  if (data.development !== void 0 && checkObject(errors, data.development, "/development")) {
    checkKnownKeys(errors, data.development, /* @__PURE__ */ new Set(["runtime_versions"]), "/development");
    if (data.development.runtime_versions !== void 0 && checkObject(errors, data.development.runtime_versions, "/development/runtime_versions")) {
      for (const [runtime, version] of Object.entries(data.development.runtime_versions)) {
        ...
        if (!RUNTIMES.has(runtime)) {
          errors.add(path, `is not a supported runtime; expected one of: ${[...RUNTIMES].join(", ")}`);
          continue;
        }
        if (typeof version !== "string" || !/^\d+(?:\.\d+){0,2}$/.test(version)) {
          errors.add(path, "must be a numeric version such as 8 or 8.0.408");
          continue;
        }
  ...
  if (data.conventions !== void 0 && checkObject(errors, data.conventions, "/conventions")) {
    for (const [key, value] of Object.entries(data.conventions)) {
      ...
      if (!/^[a-z][a-z0-9_]*$/.test(key)) {
        errors.add(path, "must be a snake_case convention name");
        continue;
      }
...
async function loadProjectConfig({ project_root, path = "adw.yaml" }) {
  ...
  try {
    ...
    if (error.code === "ENOENT") throw new PathError(`project configuration does not exist: ${path}`);
    throw error;
  }
...
function validatePlanApproval(approval) {
  ...
  checkKnownKeys(errors, approval, /* @__PURE__ */ new Set(["version", "change_id", "plan_path", "plan_digest", "plan_commit", "approved_by", "approved_at", "status", "superseded_at", "superseded_reason"]), "");
  ...
  if (approval.status === "active" && (approval.superseded_at !== void 0 || approval.superseded_reason !== void 0)) {
  ...
  if (approval.status === "superseded") {
    ...
    checkSingleLine(errors, approval.superseded_reason, "/superseded_reason", 1e3);
  }
  return { valid: errors.valid, errors: errors.items };
}
function createPlanApproval({ change_id, plan_path, plan_digest, plan_commit, approved_by, approved_at }) {
  const approval = {
    version: 1,
    change_id,
...
function supersedePlanApproval(approval, { reason, superseded_at }) {
...
function verifyPlanApproval({ approval, plan_bytes, plan_digest, plan_commit, change_id, plan_path }) {
  const validation = validatePlanApproval(approval);
  ...
  const currentDigest = plan_bytes !== void 0 ? computeDigest(plan_bytes) : plan_digest;
  ...
  if (change_id !== void 0 && change_id !== approval.change_id) return { verified: false, reason: "approval belongs to a different change", errors: [] };
  if (plan_path !== void 0 && plan_path !== approval.plan_path) return { verified: false, reason: "approval binds a different plan path", errors: [] };
  if (plan_commit !== void 0 && plan_commit !== approval.plan_commit) return { verified: false, reason: "approval is bound to a different docs commit", errors: [] };
  return { verified: true, reason: "approval matches the exact plan bytes and docs commit", errors: [] };
}
var GROUP_KEYS = /* @__PURE__ */ new Set(["branch", "worktree", "tasks", "affected_paths", "tracker", "pull_request", "implementation_commit", "review", "validation", "status"]);
var REVIEW_STATUSES = /* @__PURE__ */ new Set(["pending", "passed", "failed"]);
var VALIDATION_STATUSES = /* @__PURE__ */ new Set(["pending", "passed", "failed"]);
var GROUP_PROGRESS = new Map(GROUP_STATUSES.map((status, index) => [status, index]));
function validateCommandEvidence(errors, item, path) {
  if (!checkObject(errors, item, path)) return;
  checkKnownKeys(errors, item, /* @__PURE__ */ new Set(["command", "cwd", "exit_code", "signal", "timed_out", "duration_ms", "summary", "required"]), path);
  checkNonEmptyString(errors, item.command, `${path}/command`);
  if (item.cwd !== void 0) checkRelativePath(errors, item.cwd, `${path}/cwd`);
  ...
  if (item.required !== void 0 && typeof item.required !== "boolean") errors.add(`${path}/required`, "must be a boolean");
  if (item.summary !== void 0 && typeof item.summary !== "string") errors.add(`${path}/summary`, "must be a string");
}
...
function validateGroupRecord(errors, group, path) {
  ...
  if (group.tracker !== null && group.tracker !== void 0) {
    if (checkObject(errors, group.tracker, `${path}/tracker`)) {
      checkKnownKeys(errors, group.tracker, /* @__PURE__ */ new Set(["provider", "operation", "external_id", "url", "status"]), `${path}/tracker`);
  ...
  if (group.pull_request !== null && group.pull_request !== void 0) {
    if (checkObject(errors, group.pull_request, `${path}/pull_request`)) {
      checkKnownKeys(errors, group.pull_request, /* @__PURE__ */ new Set(["provider", "url", "number", "state"]), `${path}/pull_request`);
  ...
  if (checkObject(errors, group.review, `${path}/review`)) {
    checkKnownKeys(errors, group.review, /* @__PURE__ */ new Set(["status", "high_findings"]), `${path}/review`);
  ...
  if (checkObject(errors, group.validation, `${path}/validation`)) {
    checkKnownKeys(errors, group.validation, /* @__PURE__ */ new Set(["status", "commands", "deferred", "recorded_at"]), `${path}/validation`);
...
function validateRunRecord(record) {
  ...
  checkKnownKeys(errors, record, /* @__PURE__ */ new Set(["version", "change_id", "phase_id", "plan_digest", "base_branch", "base_commit", "started_at", "completed_at", "status", "groups"]), "");
  ...
  for (const [id, group] of Object.entries(record.groups)) {
    ...
    if (!IDENTIFIER.test(id)) {
      errors.add(path, "group id must be lowercase alphanumeric with `-` or `_` separators");
      continue;
    }
...
function createRunRecord({ change_id, phase_id, plan_digest, base_branch, base_commit, started_at, groups }) {
...
function updateRunRecord(record, update) {
  ...
  if (update.completed_at !== void 0) next.completed_at = update.completed_at;
  ...
  for (const [id, patch] of Object.entries(update.groups ?? {})) {
    ...
    if (patch.branch !== void 0 && patch.branch !== group.branch) throw new InputError(`group ${id} branch is immutable once prepared`);
    if (patch.worktree !== void 0 && patch.worktree !== group.worktree) throw new InputError(`group ${id} worktree is immutable once prepared`);
...
function redactAndBound(text) {
  return String(text ?? "").replace(/(authorization\s*:\s*bearer\s+)[^\s]+/gi, "$1[REDACTED]").replace(/((?:password|passwd|token|api[_-]?key|secret)\s*[=:]\s*)[^\s]+/gi, "$1[REDACTED]").slice(-4e3);
}
function recordValidation({ recorded_at, commands = [], deferred = [] }) {
...
function resolveValidationCommands(sources) {
...
async function runValidationCommand(input, cwd) {
...
async function resolveProjectDirectory(projectRoot, explicitRelativePath) {
  const root = await realpath(projectRoot);
  ...
  try {
    ...
    if (error.code === "ENOENT") throw new PathError(`validation cwd does not exist: ${explicitRelativePath}`);
    throw error;
  }
...
async function applyAtomicWrites(projectRoot, operations) {
  ...
  try {
    for (let index = 0; index < operations.length; index += 1) {
      ...
      await writeFile(staged, operation.content, { encoding: "utf8", mode: destinationStat?.mode ?? 420, flag: "wx" });
...
var InputError = class extends CodedError {
  constructor(message, options) {
    super(message, EXIT.INPUT, options);
  }
};
var PathError = class extends CodedError {
  constructor(message, options) {
    super(message, EXIT.PATH_VIOLATION, options);
  }
};
var AtomicWriteError = class extends CodedError {
  constructor(message, options) {
    super(message, EXIT.ATOMIC_WRITE_FAILED, options);
  }
};
...
async function dispatch(command, rawInput) {
...
async function main() {
  ...
  try {
    ...
    throw new InputError(`stdin is not valid JSON: ${error.message}`, { cause: error });
  }
