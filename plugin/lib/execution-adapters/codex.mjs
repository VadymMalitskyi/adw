...
export function createCodexAdapter({ executable = "codex", projectRoot = process.cwd(), spawnRunner = run } = {}) {
  return { async runStage({ stage, group }) {
    ...
    const worktree = resolve(projectRoot, group.worktree);
    try { await writeFile(schemaPath, JSON.stringify(stageResultSchema(stage))); const code = await spawnRunner(executable, codexArgv({ executable, cwd: worktree, schemaPath, resultPath }), buildStagePrompt({ group, stage }), worktree); if (code !== 0) throw new ContractError("execution codex: worker exited nonzero"); return JSON.parse(await readFile(resultPath, "utf8")); }
