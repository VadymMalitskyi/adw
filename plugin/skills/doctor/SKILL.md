...
7. For schema-5 workflow policy, validate each referenced work-item profile, provider match, field coherence, safe path, and operation support. Report binding, ensure mode, stage, cardinality, profile path, and digest. Fail on a missing or invalid profile, ambiguous duplicate component paths, a workflow using a disabled capability, or an unavailable required create/link operation. Do not read business objects or create anything.
8. For schema 5, verify the `managed-development` declaration and required provider artifacts. In a managed devcontainer, also verify the root-owned Claude settings/hook, the root-owned canonical Codex rule payload, and the effective user-owned Codex rule copy against that payload. Fail on missing, symlinked, conflicting, bypass, full-access, or drifted policy; do not describe the effective Codex copy as root-owned.
9. If integrations are absent, report `lightweight: no integrations configured` and do not probe external tools.
10. Treat a missing resource, a literal unexpanded Claude variable, or a path outside the installed plugin root as a plugin failure.
11. Offer `adw:init`, `adw:update`, or a manual project edit as a separate follow-up. Make no repair during doctor.
