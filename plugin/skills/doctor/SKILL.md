...
8. For schema 5, verify the `managed-development` declaration and selected provider artifacts. In a managed devcontainer, also verify the root-owned Claude settings/hook, the root-owned canonical Codex rule payload, and the effective user-owned Codex rule copy against that payload. Fail on missing, symlinked, conflicting, bypass, full-access, or drifted policy; do not describe the effective Codex copy as root-owned.
