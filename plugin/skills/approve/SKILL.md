...
7. Validate referenced work-item profiles and invoke `resolve-project-policy` with the exact union of affected paths. Require its components, required validation, tracker policy, profile digest, and `project_policy_digest` to equal `plan.effective_policy`. A required tracker binding must exist before approval. Stop for `adw:amend` on relevant drift; unrelated project configuration does not invalidate the plan.
