...
3. Invoke the helper's `load-artifact-file` command for `plan.yaml` with `artifact: "plan"`; require exit code 0 and use only its returned parsed `data`. Preserve the separately read raw bytes for approval binding.
...
5. Require sequential task IDs, repository-grounded affected paths and anchors, documented restrictions, and structured validation descriptors containing `command`, `cwd`, `timeout_ms`, `required`, and `source`.
6. When `integrations.yaml` exists, load it with `load-artifact-file` as artifact `integration`, verify its `change_id`, and read every requirement-bearing external binding back through its configured provider. Recompute the normalized selected values with `digest-requirements` and stop if they do not match `requirements_digest`. Ignore drift only in excluded operational fields such as state or assignee.
7. Load every referenced work-item profile with `load-artifact-file` as artifact `work-item-profile` and invoke `resolve-project-policy` with the exact union of affected paths. Require its components, required validation, tracker policy, profile digest, and `project_policy_digest` to equal `plan.effective_policy`. A required tracker binding must exist before approval. Stop for `adw:amend` on relevant drift; unrelated project configuration does not invalidate the plan.
8. Require `HEAD` to be a 40-hex commit containing the exact current bytes of every approval input and no uncommitted artifact edits. This `HEAD` is `docs_commit`: the pre-approval artifact commit, not the later approval commit or a future docs-branch head.
9. If an active `approval.json` already exists, verify its complete schema-2 input bundle. Report an exact duplicate as already approved. For stale or conflicting active evidence, stop and route through `adw:amend`; do not overwrite it.
