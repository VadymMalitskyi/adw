...
5. Require sequential task IDs, repository-grounded affected paths and anchors, documented restrictions, and structured validation descriptors containing `command`, `cwd`, `timeout_ms`, `required`, and `source` for plan schema 2.
...
7. For plan schema 2, validate referenced work-item profiles and invoke `resolve-project-policy` with the exact union of affected paths. Require its components, required validation, tracker policy, profile digest, and `project_policy_digest` to equal `plan.effective_policy`. A required tracker binding must exist before approval. Stop for `adw:amend` on relevant drift; unrelated project configuration does not invalidate the plan.
...
9. If an active `approval.json` already exists, verify its complete input bundle. A legacy schema-1 approval may be verified only when `integrations.yaml` is absent. Report an exact duplicate as already approved. For stale or conflicting active evidence, stop and route through `adw:amend`; do not overwrite it.
