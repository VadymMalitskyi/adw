...
2. Resolve the project root and read the root `adw.yaml`. It is the small handwritten `adw: 1` contract; there is no project schema version to dispatch on and no artifact registry to consult. Require an attached configured docs worktree at `docs.worktree`; recommend `adw:init-greenfield` for an empty project or `adw:init-brownfield` for an established repository when initialization is incomplete.
