...
2. Resolve the project root with `git rev-parse --show-toplevel`. Enforce required isolation before project commands or any fix; preferred weaker isolation needs explicit confirmation only for fix mode. Then run `node <plugin-root>/skills/sync-docs/scripts/sync-docs.mjs report --project-root <project-root>`.
