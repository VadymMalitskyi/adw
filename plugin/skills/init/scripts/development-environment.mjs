...
function requirementsDocumentation(requirements) {
  const lines = [
    ...
    "",
    "Generated by `adw:init` from supported manifests and lockfiles. Sources identify the evidence for each decision; arbitrary project prose is never executed.",
    "",
    "## Runtimes",
    "",
  ];
...
export function discoverProjectUnderstanding(projectRoot, requirements = discoverDevelopmentEnvironment(projectRoot)) {
  ...
  const architecture = [
    "# Project context",
    "",
    "This context was generated by `adw:init` from repository manifests, dependency declarations, conventional entry points, and bounded source-code signatures. Every non-trivial statement carries a project-relative evidence path.",
    "",
...
export function discoverDevelopmentEnvironment(projectRoot, { runtimeVersions = {} } = {}) {
  ...
  for (const componentRoot of roots) {
    ...
    if (existsSync(packagePath)) {
      ...
      dependencies.push(...nodePackageReferences(projectRoot, packagePath, component, manifest, nodeInstall));
