...
4. Explain that the reviewed apply will create Git when absent, establish `main`, create a concise `PROJECT.md`, create a stable `make check` validation entry point, copy the editable standard plan template to `adw/plan-templates/standard.md`, initialize ADW, commit the first main snapshot, and create the docs branch. It will not choose or generate an application framework.
...
8. Review the exact Git action and first commit intent; `PROJECT.md`; `Makefile`; `adw.yaml`; `adw/plan-templates/standard.md`; routing, ignore, permission, and optional container files; docs files; integrations; and next steps. Ensure `adw.yaml` selects the project template and declares root component `app` with sourced validation command `make check`.
