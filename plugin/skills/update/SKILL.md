...
2. Resolve the project root with `git rev-parse --show-toplevel` and run `node <plugin-root>/skills/update/scripts/update.mjs preview --project-root <project-root>`.
   - Resolve `<plugin-root>/execution/contracts.md`. Migration to schema 3 may run outside the future required environment because it only records the existing boundary; after migration, enforce the selected profile for later mutations.
