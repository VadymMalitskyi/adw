...
4. **Artifacts:** Parse schema-5 `adw.yaml` without normalizing or rewriting it and validate the project object with the helper. Read `spec.md`, schema-2 `plan.yaml`, optional `integrations.yaml`, and schema-2 `approval.json` from the docs worktree as exact raw bytes. Validate every present artifact with the helper. Require an active approval and plugin compatibility. Previous project, plan, and approval schemas are unsupported.
...
8. **Effective project policy:** Validate referenced work-item profiles and invoke `resolve-project-policy` with the parsed schema-5 project and exact union of affected paths. Require its components, required validation, optional tracker policy, profile digest, and `project_policy_digest` to equal `plan.effective_policy`. Stop for `adw:amend` on relevant drift, newly affected or ambiguously owned paths, a missing required binding, or an unavailable required operation. Configuration never authorizes an external write.
