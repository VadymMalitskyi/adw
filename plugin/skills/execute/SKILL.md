...
3. **Base and branch:** Require the configured default branch to exist and be checked out for a new execution. Require an explicit fast-forward update decision if it is behind its upstream. Resolve exactly one `adw/<change-id>` feature branch, but do not create it until every preflight gate below passes. A new execution records its exact starting branch and commit in the branch history as described below. On resume, recover and verify that durable marker instead of inferring a base from the current default-branch tip. Never rebase or reset implicitly.
4. **Artifacts:** Load `adw.yaml`, `plan.yaml`, optional `integrations.yaml`, and referenced work-item profiles with the helper's `load-artifact-file` command and use only its parsed `data`. Separately read `spec.md`, the YAML inputs, and `approval.json` from the docs worktree as exact raw bytes for approval verification. Validate every present JSON artifact with the installed helper and require an active approval. Do not add a separate schema-compatibility or migration interpretation.
5. **Approval commit:** Require `approval.docs_commit` to name an existing 40-hex commit on the docs branch. It must contain byte-identical copies of every path in `approval.inputs`, exactly `spec.md`, `plan.yaml`, and optional `integrations.yaml` when present. It is the pre-approval artifact commit, not the approval commit or current docs `HEAD`. Require the current docs history to descend from it.
6. **Approval digest:** Invoke `verify-approval-bundle` with the current exact ordered input bytes, parsed approval, and recorded `docs_commit`. Continue only on exit code 0 and `verified: true`. Any bound-byte change, missing input, missing or superseded approval, schema error, commit mismatch, or helper failure requires reapproval; do not reproduce the digest in prose.
...
8. **Effective project policy:** Use the helper-loaded work-item profiles and invoke `resolve-project-policy` with the helper-loaded schema-5 project and exact union of affected paths. Require its components, required validation, optional tracker policy, profile digest, and `project_policy_digest` to equal `plan.effective_policy`. Stop for `adw:amend` on relevant drift, newly affected or ambiguously owned paths, a missing required binding, or an unavailable required operation. Configuration never authorizes an external write.
...
After all preflight gates pass, create or resume the execution branch:

- **New branch:** Resolve the current configured default-branch `HEAD` to a full 40-hex commit, create `adw/<change-id>` from that exact commit, and immediately create one empty local marker commit with subject `Start ADW execution <change-id>` and trailers `ADW-Change-ID: <change-id>`, `ADW-Base-Branch: <configured-default-branch>`, and `ADW-Base-Commit: <40-hex-commit>`. Disable repository hooks for this metadata commit. Require the marker commit's sole parent to equal `ADW-Base-Commit`; stop if the marker cannot be created exactly. This local metadata commit records execution state but authorizes no external action or push.
- **Resume:** Require exactly one reachable commit with that subject and those three trailers. Require its change ID and base branch to match the requested change and current configuration, its recorded base to exist locally, its sole parent to equal the recorded base, feature `HEAD` to descend from the marker commit, and the configured default branch to contain the recorded base. Reject missing, duplicate, malformed, rebased, or forged markers. Inspect every commit and the complete diff from `ADW-Base-Commit` through feature `HEAD` before continuing.

Use the verified `ADW-Base-Commit` as the exact base everywhere below. Advancement of the default branch does not rewrite this recorded base; if new upstream work requires a rebase, merge, behavior change, or scope change, stop and require an explicit new execution decision or amendment instead of moving the marker.
