...
4. **Artifacts:** Load schema-5 `adw.yaml`, schema-2 `plan.yaml`, optional `integrations.yaml`, and referenced work-item profiles with the helper's `load-artifact-file` command and use only its parsed `data`. Separately read `spec.md`, the YAML inputs, and schema-2 `approval.json` from the docs worktree as exact raw bytes for approval verification. Validate every present JSON artifact with the helper. Require an active approval and plugin compatibility. Previous project, plan, and approval schemas are unsupported.
