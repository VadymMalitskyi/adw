#!/usr/bin/env node
// Deterministic Git mechanics for phased ADW execution.
//
// This script prepares and inspects one isolated branch and worktree per
// execution group. It never spawns agents, implements tasks, commits
// implementation work, pushes, opens pull requests, or mutates trackers. The
// coordinating agent owns all of that; this script only makes the Git state
// exact, resumable, and confined.
import { createHash } from "node:crypto";
import { spawnSync } from "node:child_process";
import { existsSync, lstatSync, mkdirSync, readdirSync, realpathSync } from "node:fs";
import { dirname, isAbsolute, join, relative, resolve, sep } from "node:path";

const ACTIONS = new Set(["preview", "prepare", "inspect", "cleanup-guidance"]);
...
function requireRelativePath(value, label) {
  ...
  if (isAbsolute(value) || value.includes("\0") || value.includes("\\")) throw new Error(`${label} must be an explicit project-relative path: ${value}`);
  if (value.split("/").includes("..")) throw new Error(`${label} must not contain a \`..\` segment: ${value}`);
...
function assertInsideProject(projectRoot, relativePath, label) {
  ...
  if (!rel || rel === ".." || rel.startsWith(`..${sep}`) || isAbsolute(rel)) throw new Error(`${label} escapes the project root: ${relativePath}`);
  // Reject a symlinked ancestor so a prepared worktree cannot be redirected.
  let current = projectRoot;
  for (const part of rel.split(sep)) {
    current = join(current, part);
    if (existsSync(current) && lstatSync(current).isSymbolicLink()) throw new Error(`${label} cannot be created through a symbolic link: ${relativePath}`);
  }
  return target;
}
...
function normalizeInput(raw) {
  if (!isObject(raw)) throw new Error("orchestrator input must be a JSON object");
  const projectRoot = realpathSync(String(raw.project_root ?? ""));
  const top = git(projectRoot, ["rev-parse", "--show-toplevel"]);
  if (realpathSync(top.stdout) !== projectRoot) throw new Error("project_root must be the Git top level");
  if (typeof raw.change_id !== "string" || !CHANGE_ID.test(raw.change_id)) throw new Error("change_id must be a safe change id");
  if (typeof raw.phase_id !== "string" || !IDENTIFIER.test(raw.phase_id)) throw new Error("phase_id must be a safe phase id");
  if (typeof raw.plan_digest !== "string" || !SHA256.test(raw.plan_digest)) throw new Error("plan_digest must be the sha256 digest of the approved plan bytes");
  if (typeof raw.base_branch !== "string" || raw.base_branch.length === 0 || /\s/.test(raw.base_branch)) throw new Error("base_branch is required");
  if (typeof raw.base_commit !== "string" || !COMMIT.test(raw.base_commit)) throw new Error("base_commit must be a 40-hex commit");
  if (!Array.isArray(raw.groups) || raw.groups.length === 0) throw new Error("at least one group is required");
  ...
  for (const item of raw.groups) {
    if (!isObject(item)) throw new Error("each group must be an object");
    const groupId = item.group_id;
    if (typeof groupId !== "string" || !IDENTIFIER.test(groupId)) throw new Error(`group_id must be a safe identifier: ${groupId}`);
    if (ids.has(groupId)) throw new Error(`duplicate group id: ${groupId}`);
    ids.add(groupId);
    const tasks = item.tasks ?? [];
    if (!Array.isArray(tasks) || tasks.length === 0 || tasks.some((task) => typeof task !== "string" || task.length === 0)) {
      throw new Error(`group ${groupId} requires a non-empty list of interpreted task directives`);
    }
    const affected = (item.affected_paths ?? []).map((value, index) => requireRelativePath(value, `group ${groupId} affected_paths[${index}]`));
    if (affected.length === 0) throw new Error(`group ${groupId} requires at least one affected path`);
    const validation = (item.validation ?? []).map((entry, index) => {
      if (typeof entry === "string") return { command: entry, cwd: ".", required: true };
      if (!isObject(entry) || typeof entry.command !== "string" || entry.command.length === 0) throw new Error(`group ${groupId} validation[${index}] requires a command`);
      return { command: entry.command, cwd: requireRelativePath(entry.cwd ?? ".", `group ${groupId} validation[${index}].cwd`) || ".", required: entry.required !== false, ...(entry.timeout_ms ? { timeout_ms: entry.timeout_ms } : {}) };
    });
    ...
    if (branches.has(branch)) throw new Error(`duplicate group branch: ${branch}`);
    if (worktrees.has(worktree)) throw new Error(`duplicate group worktree: ${worktree}`);
    branches.add(branch);
  ...
  return { projectRoot, changeId: raw.change_id, phaseId: raw.phase_id, planDigest: raw.plan_digest, baseBranch: raw.base_branch, baseCommit: raw.base_commit, groups };
}
...
// a shared file must place it in an earlier phase's contract group.
    ...
    for (let right = left + 1; right < groups.length; right += 1) {
      for (const leftPath of groups[left].affected_paths) {
        for (const rightPath of groups[right].affected_paths) {
          ...
          throw new Error(`groups ${groups[left].group_id} and ${groups[right].group_id} both write ${leftPath === rightPath ? leftPath : `${leftPath} and ${rightPath}`}; parallel groups require disjoint write paths`);
        }
      }
    }
  }
...
// session never has to trust chat history or an unverified run record.
...
function assertBase(projectRoot, context) {
  ...
  if (type.status !== 0 || type.stdout !== "commit") throw new Error(`base_commit does not exist in this repository: ${context.baseCommit}`);
  if (!branchExists(projectRoot, context.baseBranch)) throw new Error(`base_branch does not exist locally: ${context.baseBranch}`);
  const contains = git(projectRoot, ["merge-base", "--is-ancestor", context.baseCommit, context.baseBranch], { allowFailure: true });
  if (contains.status !== 0) throw new Error(`base_branch ${context.baseBranch} does not contain base_commit ${context.baseCommit}`);
}
...
function prepareGroup(projectRoot, context, group, state) {
  ...
  try {
    ...
    if (parents.length !== 1 || parents[0] !== context.baseCommit) throw new Error(`group ${group.group_id} marker commit did not land directly on ${context.baseCommit}`);
...
function cleanupGuidance(context, groups) {
  return groups.map((group) => ({
    group_id: group.group_id,
    // ADW never removes a branch or worktree by itself; a human decides when
    // the work in them is genuinely disposable.
    commands: [
      `git worktree remove ${group.worktree}`,
      `git branch -d ${group.branch}`,
    ],
...
try {
  ...
  if (!ACTIONS.has(action)) fail(`unknown action ${JSON.stringify(action)}; expected ${[...ACTIONS].join(", ")}`);
  const context = normalizeInput(await readStdin());
  const { projectRoot } = context;
  assertBase(projectRoot, context);

  const states = context.groups.map((group) => inspectGroup(projectRoot, context, group));
  const blocked = states.filter((state) => state.blockers.length > 0);

  if (action === "cleanup-guidance") {
    process.stdout.write(`${JSON.stringify({ ok: true, action, groups: cleanupGuidance(context, context.groups) }, null, 2)}\n`);
  } else if (action === "prepare") {
    if (blocked.length > 0) {
      process.stdout.write(`${JSON.stringify({ ok: false, action, error: "one or more groups are blocked; resolve them before preparing", groups: states }, null, 2)}\n`);
      process.exitCode = 2;
    } else {
      const prepared = [];
      for (const [index, group] of context.groups.entries()) prepared.push(prepareGroup(projectRoot, context, group, states[index]));
      process.stdout.write(`${JSON.stringify({
        ...
        groups: prepared,
        cleanup: cleanupGuidance(context, context.groups),
      }, null, 2)}\n`);
    }
  } else {
    process.stdout.write(`${JSON.stringify({
