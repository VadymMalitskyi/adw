...
// The managed permission profile is a release-owned constant. It is never
// stored in `adw.yaml`: callers learn that the hardened variant is in effect
// from `execution.isolation`, and the ordinary project permission files below
// are written for every isolation mode.
export const PERMISSION_PROFILE = "managed-development";

// `managed-devcontainer` implies the hardened managed profile; every other
// isolation mode still receives the same project permission rules without the
// container-owned root policy, wrappers, and firewall.
export function permissionProfileForIsolation(isolation) {
  return { profile: PERMISSION_PROFILE, managed: isolation === "managed-devcontainer" };
}
