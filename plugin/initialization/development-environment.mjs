...
function dotnetTargetFrameworkVersion(projectRoot, projectPath) {
  ...
  const frameworks = [
    ...text.matchAll(/<TargetFramework(?:s)?>\s*([^<]+?)\s*<\/TargetFramework(?:s)?>/gi),
  ].flatMap((match) => match[1].split(";"));
  const versions = [...new Set(frameworks
    .map((framework) => framework.trim().match(/^net(\d+(?:\.\d+)?)(?:[-.]|$)/i)?.[1])
    .filter(Boolean))];
...
function dotnetPackageReferences(projectRoot, projectPath, component) {
  const text = readFileSync(projectPath, "utf8");
  const dependencies = [];
  const element = /<PackageReference\b([^>]*?)(?:\/>|>([\s\S]*?)<\/PackageReference>)/gi;
  for (const match of text.matchAll(element)) {
    const attributes = match[1] ?? "";
    const name = attributes.match(/\bInclude\s*=\s*["']([^"']+)["']/i)?.[1];
    const version = attributes.match(/\bVersion\s*=\s*["']([^"']+)["']/i)?.[1]
      ?? match[2]?.match(/<Version>\s*([^<\s][^<]*?)\s*<\/Version>/i)?.[1];
    if (!name) continue;
    dependencies.push({
      ecosystem: "nuget",
      name,
      ...(version ? { version } : {}),
      component,
      source: sourcePath(projectRoot, projectPath),
      install: "dotnet restore",
    });
  }
  return dependencies.sort((left, right) => compareText(left.name, right.name) || compareText(left.version ?? "", right.version ?? ""));
}

function nodePackageReferences(projectRoot, packagePath, component, manifest, install) {
  ...
  return Object.entries(declared)
    ...
    .map(([name, version]) => ({ ecosystem: "npm", name, version, component, source: sourcePath(projectRoot, packagePath), install }))
    .sort((left, right) => compareText(left.name, right.name));
}

function pythonPackageReferences(projectRoot, requirementsPath, component) {
  const dependencies = [];
  for (const line of readFileSync(requirementsPath, "utf8").split(/\r?\n/)) {
    const match = /^\s*([A-Za-z0-9][A-Za-z0-9._-]*)(?:\[[^\]]+\])?\s*([!<>=~].+)?\s*$/.exec(line);
    if (!match || line.trim().startsWith("#")) continue;
    dependencies.push({ ecosystem: "pip", name: match[1], ...(match[2] ? { version: match[2].trim() } : {}), component, source: sourcePath(projectRoot, requirementsPath), install: "python -m pip install -r requirements.txt" });
  }
  return dependencies.sort((left, right) => compareText(left.name, right.name));
}

function goModuleReferences(projectRoot, goPath, component) {
  const text = readFileSync(goPath, "utf8");
  const dependencies = [];
  const add = (name, version) => {
    if (!name || name.startsWith("//")) return;
    dependencies.push({ ecosystem: "go", name, ...(version ? { version } : {}), component, source: sourcePath(projectRoot, goPath), install: "go mod download" });
  };
  for (const match of text.matchAll(/^require\s+(\S+)\s+(\S+)/gm)) add(match[1], match[2]);
  const blocks = [...text.matchAll(/^require\s*\(([^)]*)\)/gms)];
  for (const block of blocks) for (const line of block[1].split(/\r?\n/)) {
...
function requirementsDocumentation(requirements) {
  const lines = [
    "# Project development requirements",
    "",
    "Generated by an ADW initialization workflow from supported manifests, lockfiles, and explicit greenfield runtime choices. Sources identify the evidence for each decision; arbitrary project prose is never executed.",
    "",
    "## Runtimes",
    "",
  ];
  if (Object.keys(requirements.selected_versions).length === 0) lines.push("No deterministic runtime versions were detected.");
  else for (const [name, version] of Object.entries(requirements.selected_versions)) lines.push(`- \`${name}\` ${version}`);
  lines.push("", "## Application dependencies", "");
  if (requirements.dependencies.length === 0) lines.push("No supported manifest dependencies were detected.");
  else for (const dependency of requirements.dependencies) lines.push(`- \`${dependency.name}\`${dependency.version ? ` ${dependency.version}` : ""} (${dependency.ecosystem}; restored by \`${dependency.install}\`; source: \`${dependency.source}\`)`);
  lines.push("", "## Native packages", "");
  if (requirements.system_packages.length === 0) lines.push("No additional native packages were detected.");
  else for (const item of requirements.system_packages) lines.push(`- \`${item.name}\` (source: \`${item.source}\`)`);
  lines.push("", "## Setup commands", "");
  if (requirements.setup_commands.length === 0) lines.push("No lockfile- or manifest-backed setup commands were generated.");
  else for (const item of requirements.setup_commands) lines.push(`- \`${item.command}\` (source: \`${item.source}\`)`);
  if (requirements.unresolved.length > 0) {
    lines.push("", "## Needs attention", "");
    for (const item of requirements.unresolved) lines.push(`- ${item.requirement}: ${item.reason} (source: \`${item.source}\`)`);
  }
  return `${lines.join("\n")}\n`;
}
...
function componentSlug(component) {
  ...
  return component.toLowerCase().replaceAll("\\", "/").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
}

function componentFiles(projectRoot, componentRoot, nestedComponentRoots = []) {
  const files = [];
  const nested = new Set(nestedComponentRoots.map((path) => resolve(path)));
  function visit(directory, depth) {
    if (depth > 4 || files.length >= 500) return;
    for (const entry of sortedDirectoryEntries(directory)) {
      if (entry.isSymbolicLink() || IGNORED_DIRECTORIES.has(entry.name)) continue;
      const path = join(directory, entry.name);
      if (entry.isDirectory() && nested.has(resolve(path))) continue;
      if (entry.isDirectory()) visit(path, depth + 1);
  ...
  return files.filter((path) => withinProject(projectRoot, path));
}

function sourceCapabilities(projectRoot, files, dependencies) {
  const capabilities = [];
  const add = (name, source) => addUnique(capabilities, "name", { name, source });
  for (const dependency of dependencies) {
    if (/duckdb/i.test(dependency.name)) add("Embedded DuckDB persistence", dependency.source);
    if (/entityframework/i.test(dependency.name)) add("Entity Framework persistence", dependency.source);
    if (/signalr/i.test(dependency.name)) add("SignalR realtime communication", dependency.source);
    if (/react/i.test(dependency.name)) add("React user interface", dependency.source);
  }
  const patterns = [
    { pattern: /WebApplication\.CreateBuilder\s*\(/, name: "ASP.NET Core web application" },
    { pattern: /:\s*BackgroundService\b/, name: "Background processing" },
    { pattern: /\bDuckDBConnection\b/, name: "Embedded DuckDB persistence" },
    { pattern: /:\s*Hub\b|MapHub\s*</, name: "SignalR realtime communication" },
    { pattern: /\bDbContext\b/, name: "Entity Framework persistence" },
    { pattern: /createRoot\s*\(|ReactDOM/, name: "React user interface" },
  ];
  for (const path of files) {
    let text;
    try {
    ...
    for (const candidate of patterns) if (candidate.pattern.test(text)) add(candidate.name, sourcePath(projectRoot, path));
  }
  return capabilities.sort((left, right) => compareText(left.name, right.name) || compareText(left.source, right.source));
}

function componentRelationships(projectRoot, files) {
  const relationships = [];
  for (const path of files.filter((candidate) => DOTNET_PROJECT_EXTENSION.test(candidate))) {
    const text = readFileSync(path, "utf8");
    for (const match of text.matchAll(/<ProjectReference\b[^>]*\bInclude\s*=\s*["']([^"']+)["'][^>]*\/?\s*>/gi)) {
      const target = resolve(dirname(path), match[1].replaceAll("\\", "/"));
      if (withinProject(projectRoot, target)) relationships.push({ type: "project-reference", target: sourcePath(projectRoot, target), source: sourcePath(projectRoot, path) });
    }
  }
  return relationships.sort((left, right) => compareText(left.target, right.target));
}

function authoritativeDocumentation(projectRoot) {
  const found = [];
  function visit(directory, depth) {
    ...
    for (const entry of sortedDirectoryEntries(directory)) {
      ...
      const path = join(directory, entry.name);
      if (entry.isDirectory()) visit(path, depth + 1);
      else if (/^(?:README|CONTRIBUTING|ARCHITECTURE|CHANGELOG)(?:\.[^.]+)?\.md$/i.test(entry.name) || (relative(projectRoot, path).replaceAll("\\", "/").startsWith("docs/") && /\.md$/i.test(entry.name))) found.push(sourcePath(projectRoot, path));
    }
  }
  visit(projectRoot, 0);
...
function renderComponentDocumentation(component) {
  const lines = [
    `# ${component.name}`,
    "",
    `Path: \`${component.path}\``,
  ...
  if (component.capabilities.length === 0) lines.push("No supported code signature identified a more specific role.");
  else for (const item of component.capabilities) lines.push(`- ${item.name} (evidence: \`${item.source}\`)`);
  lines.push("", "## Runtimes and manifests", "");
  if (component.runtimes.length === 0) lines.push("No runtime manifest was detected.");
  else for (const runtime of component.runtimes) lines.push(`- \`${runtime.name}\`${runtime.version ? ` ${runtime.version}` : " unpinned"} (source: \`${runtime.source}\`)`);
  for (const manifest of component.manifests) lines.push(`- Manifest: \`${manifest}\``);
  lines.push("", "## Important entry points", "");
  if (component.entry_points.length === 0) lines.push("No conventional entry point was detected.");
  else for (const path of component.entry_points) lines.push(`- \`${path}\``);
  lines.push("", "## Direct dependencies", "");
  if (component.dependencies.length === 0) lines.push("No supported direct dependency declarations were detected.");
  else for (const dependency of component.dependencies) lines.push(`- \`${dependency.name}\`${dependency.version ? ` ${dependency.version}` : ""} (${dependency.ecosystem}; source: \`${dependency.source}\`)`);
  lines.push("", "## Component relationships", "");
  if (component.relationships.length === 0) lines.push("No supported component relationship declaration was detected.");
  else for (const relationship of component.relationships) lines.push(`- ${relationship.type}: \`${relationship.target}\` (source: \`${relationship.source}\`)`);
  return `${lines.join("\n")}\n`;
}

export function discoverProjectUnderstanding(projectRoot, requirements = discoverDevelopmentEnvironment(projectRoot)) {
  const components = [];
  const roots = componentRoots(projectRoot);
  for (const componentRoot of roots) {
    const component = relative(projectRoot, componentRoot) || ".";
    const nestedRoots = roots.filter((candidate) => candidate !== componentRoot && candidate.startsWith(`${componentRoot}${sep}`));
    const files = componentFiles(projectRoot, componentRoot, nestedRoots);
    const runtimes = requirements.runtimes.filter((runtime) => runtime.component === component);
    const dependencies = requirements.dependencies.filter((dependency) => dependency.component === component);
    const manifests = files
      .filter((path) => MANIFESTS.has(path.split(sep).at(-1)) || DOTNET_PROJECT_EXTENSION.test(path))
      .map((path) => sourcePath(projectRoot, path))
      .sort(compareText);
    const sourceFiles = files.filter((path) => SOURCE_EXTENSIONS.has(`.${path.split(".").at(-1).toLowerCase()}`));
    if (runtimes.length === 0 && dependencies.length === 0 && manifests.length === 0 && sourceFiles.length === 0) continue;
    const entryPoints = sourceFiles
      .filter((path) => /(?:^|\/)(?:Program\.cs|main\.[A-Za-z0-9]+|index\.[A-Za-z0-9]+|App\.[A-Za-z0-9]+|server\.[A-Za-z0-9]+)$/i.test(sourcePath(projectRoot, path).replaceAll("\\", "/")))
      .map((path) => sourcePath(projectRoot, path))
      .slice(0, 20);
    components.push({
      name: component === "." ? "Root workspace" : component.split(sep).at(-1),
      path: component,
      ...
      dependencies,
      capabilities: sourceCapabilities(projectRoot, sourceFiles, dependencies),
      relationships: componentRelationships(projectRoot, files),
    });
  }
  const documentation = authoritativeDocumentation(projectRoot);
  const architecture = [
    "# Project context",
    "",
    "This context was generated by `adw:init-brownfield` from repository manifests, dependency declarations, conventional entry points, and bounded source-code signatures. Every non-trivial statement carries a project-relative evidence path.",
    "",
    "## Component boundaries",
    "",
    ...(components.length === 0 ? ["No source component was detected."] : components.map((component) => `- [${component.name}](components/${component.slug}.md) — \`${component.path}\`${component.capabilities.length > 0 ? `; ${[...new Set(component.capabilities.map((item) => item.name))].join(", ")}` : ""}`)),
    "",
    "## Environment model",
    "",
    ...Object.entries(requirements.selected_versions).map(([name, version]) => `- Runtime: \`${name}\` ${version}`),
    ...requirements.system_packages.map((item) => `- Native package: \`${item.name}\` (source: \`${item.source}\`)`),
    ...requirements.setup_commands.map((item) => `- Setup: \`${item.command}\` (source: \`${item.source}\`)`),
    "",
    "## Authoritative documentation",
    "",
    ...(documentation.length === 0 ? ["No conventional project documentation was detected."] : documentation.map((path) => `- \`${path}\``)),
    "",
    "## Unresolved requirements",
    "",
    ...(requirements.unresolved.length === 0 ? ["None detected."] : requirements.unresolved.map((item) => `- ${item.requirement}: ${item.reason} (source: \`${item.source}\`)`)),
    "",
  ].join("\n");
  return {
    schema: 1,
    components,
    authoritative_documentation: documentation,
    files: new Map([
      ["README.md", "# ADW project records\n\nThe generated architecture and component documents describe the code evidence used to prepare this project's development environment. Review them together with each ADW change record.\n"],
      ["architecture.md", architecture],
      ...components.map((component) => [`components/${component.slug}.md`, renderComponentDocumentation(component)]),
    ]),
  };
}
...
export function discoverDevelopmentEnvironment(projectRoot, { runtimeVersions = {}, includeChosenRuntimes = false } = {}) {
  ...
  for (const componentRoot of roots) {
    ...
    if (existsSync(packagePath)) {
      ...
      const nodeInstall = existsSync(join(componentRoot, "pnpm-lock.yaml")) ? "corepack pnpm install --frozen-lockfile"
        : existsSync(join(componentRoot, "yarn.lock")) ? "corepack yarn install --immutable"
          : existsSync(join(componentRoot, "package-lock.json")) || (isRootWorkspace && rootHasLockedInstall) ? "npm ci"
            : "package-manager install";
      const nodeDependencies = nodePackageReferences(projectRoot, packagePath, component, manifest, nodeInstall);
      ...
      if (nodeDependencies.length === 0 && !componentHasLock && !(isRootWorkspace && rootHasLockedInstall)) {
    ...
    if (existsSync(pyprojectPath) || pythonFiles.length > 0) {
      ...
      const requirementsPath = join(componentRoot, "requirements.txt");
      if (existsSync(requirementsPath)) dependencies.push(...pythonPackageReferences(projectRoot, requirementsPath, component));
    ...
    if (existsSync(goPath)) {
      ...
      dependencies.push(...goModuleReferences(projectRoot, goPath, component));
    }
    ...
    if (dotnetProjectPath) {
      ...
      dependencies.push(...dotnetPackageReferences(projectRoot, dotnetProjectPath, component));
  ...
  return {
    ...
    dependencies: dependencies.sort((left, right) => compareText(left.component, right.component) || compareText(left.name, right.name)),
...
export function managedDevelopmentFiles(projectRoot, templateRoot, { agentTools = "both", webAccess, integrationDomains = [], runtimeVersions = {}, includeChosenRuntimes = false } = {}) {
  ...
  const requirements = discoverDevelopmentEnvironment(projectRoot, { runtimeVersions, includeChosenRuntimes });
  const requirementsText = stableJson(requirements);
  const requirementsGuide = requirementsDocumentation(requirements);
  ...
  return {
    ...
    files: new Map([
      ...
      ["project-requirements.md", requirementsGuide],
